...
class SesameSeedAugmentor:
    ...
    def save_augmented_images(self, class_name, original_images, num_augmentations=15):
        ...
        total_saved = 0
        for idx, original_image in enumerate(tqdm(original_images, desc=f"Augmenting {class_name}")):
            ...
            for aug_idx, aug_image in enumerate(augmented_images):
                ...
                filename = f"{class_name.replace(' ', '_')}_{idx:03d}_aug_{aug_idx:03d}.jpg"
    ...
    def run_augmentation(self, augmentations_per_image=15):
        ...
        for class_name, images in images_by_class.items():
            if images:
                count = self.save_augmented_images(class_name, images, augmentations_per_image)
                total_images += count
...
if __name__ == "__main__":
    ...
    try:
        ...
        import subprocess
        subprocess.check_call(["pip", "install", "albumentations", "opencv-python", "numpy", "tqdm"])
