...
class SesameSeedDetector:
    def __init__(self, model_path=None):
        ...
        if model_path is None:
            ...
            if not self.model_path.exists() or self.model_path.stat().st_size == 0:
                print("Primary model not found or corrupt. Searching for fallbacks...")
                # Try to find model in runs directory recursively
                runs_dir = self.project_dir / "runs"
                if runs_dir.exists():
                    all_best_weights = list(runs_dir.rglob("best.pt"))
                    if all_best_weights:
                        # Use the most recently modified best.pt
                        self.model_path = max(all_best_weights, key=lambda x: x.stat().st_mtime)
                        print(f"Found fallback model: {self.model_path}")
        else:
        ...
        # Detection confidence threshold (LOWERED for better recall)
    ...
    def detect_image(self, image_path, save=True, show=True):
        ...
        results = self.model.predict(
            ...
            imgsz=320,  # ALIGNED with training size
            verbose=False,
        )
        ...
        if save and isinstance(image_path, (str, Path)):
            output_path = (
                Path(image_path).parent / f"{Path(image_path).stem}_detected.jpg"
            )
    ...
    def detect_batch(self, input_dir, output_dir=None):
        ...
        for img_file in image_files:
            result = self.detect_image(img_file, save=True, show=False)
    ...
    def _add_summary_to_image(self, image, summary_text):
        ...
        for i, line in enumerate(lines):
            y = y0 + i * dy
            cv2.putText(
                image, line, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 3
            )  # Black outline
            cv2.putText(
                image, line, (10, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2
            )  # White text
...
def main():
    ...
    parser.add_argument(
        "--eval", action="store_true", help="Evaluate model on validation set"
    )
    ...
    if args.eval:
        ...
        result = detector.detect_image(args.image, save=True, show=not args.no_show)
        ...
        detector.detect_video(
            args.video, output_path=args.output, show=not args.no_show
        )
