...
def run_detection_test():
    ...
    # Find a test image
    test_images = list(Path("data").glob("**/*.jpeg")) + list(
        Path("data").glob("**/*.jpg")
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run complete Sesame Seed Detection pipeline"
    )
    parser.add_argument(
        "--install", action="store_true", help="Install requirements only"
    )
    ...
    parser.add_argument(
        "--skip-install", action="store_true", help="Skip installation check"
    )
